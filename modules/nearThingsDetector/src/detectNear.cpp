...
void DetectNear::loop()
{
	...
	Mat threshIm;
	threshold(disp, threshIm, backgroundThresh, 1, CV_THRESH_BINARY);			// Firs
	...
	while (fillSize < minBlobSize){			
		...
		fillSize = floodFill(aux, maxLoc, 0, 0, Scalar(maxVal/dispThreshRatioLow), Scalar(maxVal/dispThreshRatioHigh));	// If its too small, paint it black and search again
	}
	floodFill(disp, maxLoc, 255, 0, Scalar(maxVal/dispThreshRatioLow), Scalar(maxVal/dispThreshRatioHigh));	// Paint closest valid blob white
...
{
	range = rf.check("range", Value(0.5)).asDouble();
	backgroundThresh = rf.check("backgroundThresh", Value(50)).asInt();		// threshold of intensity if the disparity image, under which info is ignored.
    cannyThresh = rf.check("cannyThresh", Value(20)).asDouble();
	minBlobSize = rf.check("minBlobSize", Value(400)).asInt();
	gaussSize = rf.check("gaussSize", Value(5)).asInt();
    dispThreshRatioLow = rf.check("dispThreshRatioLow", Value(20)).asInt();    
    dispThreshRatioHigh = rf.check("dispThreshRatioHigh", Value(40)).asInt();
