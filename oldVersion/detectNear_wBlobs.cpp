...
void DetectNear::loop()
{
	...
	if(blobBoxes->size() > 0){
		...
		Rect blob(0,0,disparity->width(),disparity->height());	// Select whole image as blob	
		blobRects.push_back(blob);
	}
	...
	for( int r = 0; r < blobRects.size(); r++ ){
		rectangle(imOut, blobRects[r], Scalar(255,0,0) );
		vector<vector<Point > > contours_aux;
		findContours( edges(blobRects[r]), contours_aux, hierarchy, CV_RETR_EXTERNAL, CV_CHAIN_APPROX_SIMPLE, Point(0, 0) );
		cout << "Finding large blobs... "<< endl;
		for( int c = 0; c < contours_aux.size(); c++ ){
			double a = contourArea(contours_aux[c]);						// Find the area of contour
			if(a > minBlobSize){											// Keep only big contours
				// Add offset to contours_aux, because they have been saved on the coordinates of the bounding box
				vector<Point> cntDispl (contours_aux[c].size());
				for ( int p = 0; p < contours_aux[c].size(); p++ )		{
					cntDispl[p].x = contours_aux[c][p].x + blobRects[r].tl().x;
					cntDispl[p].y = contours_aux[c][p].y + blobRects[r].tl().y;
				}
				contours.push_back(cntDispl);					
			}
		}
	}
	...
	// Compute average distance of each contour/blob after cleaning all non-valid (zeros or out of contour) with masks
